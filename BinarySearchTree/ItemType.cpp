#include "ItemType.h"


// Set student id from keyboard.
void ItemType::SetIdFromKB()
{
	cout << "\tID : ";
	cin >> m_Id;
}


// Set student name from keyboard.
void ItemType::SetNameFromKB()
{
	cout << "\tName : ";
	cin >> m_sName;
}


// Set student address from keyboard.
void ItemType::SetAddressFromKB()
{
	cout << "\tAddress : ";
	cin >> m_sAddress;
}


// Set student record from keyboard.
void ItemType::SetRecordFromKB()
{
	SetIdFromKB();
	SetNameFromKB();
	SetAddressFromKB();
}


// Read a record from file.
int ItemType::ReadDataFromFile(ifstream& fin)
{
	fin >> m_Id;
	fin >> m_sName;
	fin >> m_sAddress;

	return 1;
};


// Write a record into file.
int ItemType::WriteDataToFile(ofstream& fout)
{
	fout << endl;
	fout << m_Id << " ";
	fout << m_sName << " ";
	fout << m_sAddress;

	return 1;
}


ostream& ItemType::operator<<(ostream& cout) {
	return cout << this->m_sName << endl;
}

bool ItemType::operator >(const ItemType& data) {
	if (data.m_sName.compare(this->m_sName) == 1)
		return 1;
	else
		return 0;
}

bool ItemType::operator <(const ItemType& data) {
	if (data.m_sName.compare(this->m_sName) == 0)
		return 1;
	else
		return 0;
}

bool ItemType::operator ==(const ItemType& data) {
	if (data.m_sName.compare(this->m_sName) == 0)
		return 1;
	else
		return 0;
}

bool ItemType::operator !=(const ItemType& data) {
	if(this->m_sName.compare(data.m_sName) != 0)
		return 1;
	else
		return 0;
}
void ItemType::operator =(const ItemType& data) {
	this->m_Id = data.m_Id;
	this->m_sAddress = data.m_sAddress;
	this->m_sName = data.m_sName;
}